...
class LocalRemindersService {
  ...
  async getSystemStatus(): Promise<SystemStatus> {
    ...
    return { canSchedule, permissionsGranted, exactAlarmAvailable, batteryOptimized, issues };
  }
    ...
    try {
      if (Platform.OS === 'android') {
        ...
        if (apiLevel !== null && apiLevel >= API_LEVEL_ANDROID_15) {
          ...
          console.warn('Android 15+: Private Space may affect notification delivery.');
        }
      }
    } catch (error) {
  ...
  async getComprehensiveNotificationState(): Promise<NotificationState> {
    ...
    const programmed = await Preferences.get({ key: NOTIFICATION_PROGRAMMED_KEY });
