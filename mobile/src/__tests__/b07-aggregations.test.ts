...
describe('mean', () => {
  it('calculates the mean across two lists', () => {
    const list1: MeasurementEntry[] = [{ '2024-01-15': 10 }, { '2024-01-16': 20 }];
  ...
  it('returns undefined for null inputs', () => {
    expect(
      mean(
        undefined as unknown as MeasurementEntry[],
        [{ '2024-01-15': 10 }],
      ),
    ).toBeUndefined();
  });
  ...
  it('rounds the result', () => {
    const list1: MeasurementEntry[] = [{ '2024-01-15': 1 }, { '2024-01-16': 2 }];
...
describe('calculateMeasurement', () => {
  const historicalData: HistoricalMeasurement = {
    ...
    humedad: [
      { '2024-01-15T08:00:00Z': 65 },
      { '2024-01-16T09:00:00Z': 70 },
    ],
  };

  describe('with sum', () => {
    it('sums all values for the same date', () => {
      const result = calculateMeasurement(historicalData, ['temperatura'], 'sum');
      // 2024-01-15: 22 + 28 = 50
  ...
  describe('with mean', () => {
    it('calculates the mean for multiple values on the same date', () => {
      const result = calculateMeasurement(historicalData, ['temperatura'], 'mean');
...
describe('calculateDetailedMeasurement', () => {
  ...
  it('calculates avg, min, max per day', () => {
    const result = calculateDetailedMeasurement(historicalData, ['temperatura']);
  ...
  it('handles multiple keys (merges by date)', () => {
    const multi: HistoricalMeasurement = {
      ...
      humedad: [
        { '2024-01-15T09:00:00Z': 65 },
      ],
    };
    const result = calculateDetailedMeasurement(multi, ['temperatura', 'humedad']);
...
describe('calculateOverallStats', () => {
  const historicalData: HistoricalMeasurement = {
    ...
    lluvia: [
      { '2024-01-15T10:00:00Z': 3 },
      { '2024-01-16T10:00:00Z': 7 },
    ],
  };
  ...
  const lineMeasurement = {
    ...
    style: { backgroundColor: { colorName: 'blue', colorHex: '#00f' }, borderColor: { colorName: 'blue', colorHex: '#00f' } },
    graph: {
      type: 'line',
      measurementIds: ['temperatura'],
      aggregationFunction: 'mean',
