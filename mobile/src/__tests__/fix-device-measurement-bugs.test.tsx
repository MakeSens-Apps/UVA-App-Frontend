  ...
  useTheme: () => ({
    theme: {
      colors: {
        ...
        white: '#FFFFFF',
        gray: { 50: '#FAFAFA', 100: '#f3f4f6', 200: '#e5e7eb', 300: '#d1d5db', 400: '#9ca3af', 500: '#6b7280', 600: '#525252', 700: '#404040' },
...
jest.mock('@/data/datastore/measurement-ds', () => ({
  MeasurementDSService: { addMeasurement: jest.fn().mockResolvedValue(undefined) },
}));
...
function makeRegisterProps() {
  ...
  const route = { params: { taskId: 'task1', taskName: 'Registro máximos', flowId: 'flow1' } };
...
describe('F-08 — RegisterMeasurementScreen digit input (clipped digits on Android)', () => {
  ...
  it('keeps the original visual contract: 26px font, 40px wide, 2px bottom rule', async () => {
    ...
    const style = flattenStyle<TextStyle>(getByTestId('digit-input-0-0').props.style);
