...
describe('GamificationService.completeTaskProcess', () => {
  it('adds +1 seed on first task (completedTasks was 0)', async () => {
    ...
    const updatedProgress = makeProgress({ completedTasks: 1, Seed: 6, Streak: 2 });
  ...
  it('adds +1 seed and +1 streak when all tasks complete', async () => {
    ...
    const updatedProgress = makeProgress({ completedTasks: 3, Seed: 11, Streak: 6 });
    ...
    const updatedAfterBonus = makeProgress({ completedTasks: 3, Seed: 11, Streak: 6 });
  ...
  it('middle task (not first, not last) adds no seed or streak', async () => {
    ...
    const updatedProgress = makeProgress({ completedTasks: 2, Seed: 5, Streak: 3 });
  ...
  it('retries x3 with backoff on update failure then returns false', async () => {
    ...
    mockUPS.updateUserProgress.mockResolvedValue(undefined as unknown as ReturnType<typeof mockUPS.updateUserProgress> extends Promise<infer T> ? T : never);
    ...
    expect(mockUPS.getLastUserProgressPure.mock.calls.length).toBeGreaterThanOrEqual(3);
  ...
  it('triggers streakBonus every 7 days', async () => {
    ...
    const updatedProgress = makeProgress({ completedTasks: 3, Seed: 11, Streak: 7 });
  ...
  it('creates streak progress alert when streak is multiple of 3 but not 7', async () => {
    ...
    const updatedProgress = makeProgress({ completedTasks: 3, Seed: 11, Streak: 3 });
...
describe('GamificationService.recoverStreak', () => {
  ...
  it('deducts 5 seeds and restores streak when seeds >= 5', async () => {
    ...
    const twoDaysAgoProgress = [makeProgress({ id: 'two-days-ago-id', Streak: 2 })];
