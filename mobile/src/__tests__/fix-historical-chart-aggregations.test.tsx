...
const RAW_MEASUREMENTS: { ts: string; data: Record<string, number> }[] = [
  ...
  { ts: '2026-05-01T06:00:00.000Z', data: { TEMPERATURA_MIN: 21, HUMEDAD_MIN: 60 } },
  { ts: '2026-05-01T18:00:00.000Z', data: { TEMPERATURA_MAX: 31, HUMEDAD_MAX: 86 } },
  { ts: '2026-05-01T18:05:00.000Z', data: { PRECIPITACION: 10 } },
  { ts: '2026-05-01T22:00:00.000Z', data: { PRECIPITACION: 5 } },
  // Día 2
  { ts: '2026-05-02T06:00:00.000Z', data: { TEMPERATURA_MIN: 22, HUMEDAD_MIN: 56 } },
  { ts: '2026-05-02T18:00:00.000Z', data: { TEMPERATURA_MAX: 28, HUMEDAD_MAX: 80 } },
  { ts: '2026-05-02T19:00:00.000Z', data: { PRECIPITACION: 30 } },
  // Día 3 — sólo máximos (día incompleto) y sin lluvia registrada
  { ts: '2026-05-03T18:00:00.000Z', data: { TEMPERATURA_MAX: 26, HUMEDAD_MAX: 74 } },
...
const TEM = historical('Temperatura', ['TEMPERATURA_MAX', 'TEMPERATURA_MIN'], 'mean', 'line');
const HUM = historical('Humedad', ['HUMEDAD_MAX', 'HUMEDAD_MIN'], 'mean', 'line');
...
describe('D-01 — el componente dibuja SVG real (no un contenedor vacío)', () => {
  ...
  it('detailedMode: emite la banda max/min además de la línea', async () => {
    ...
    expect(countNodes(toJSON(), ['RNSVGPath', 'Path'])).toBeGreaterThanOrEqual(2);
  });
