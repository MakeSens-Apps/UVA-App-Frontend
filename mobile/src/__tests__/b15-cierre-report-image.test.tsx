...
jest.mock('@/data/datastore/uva-ds', () => ({
  UvaDSService: {
    getUVAByID: jest.fn().mockResolvedValue({ fields: JSON.stringify({ farmName: 'Finca El Paraiso' }) }),
...
jest.mock('@/theme/ThemeProvider', () => ({
  useTheme: () => ({
    theme: {
      colors: {
        blue: { 50: '#EDFEFE', 100: '#D1FBFC', 200: '#A9F5F8', 500: '#10BCCA', 600: '#1097AA', 700: '#14788A', 800: '#1A6270', 900: '#164551' },
        orange: { 500: '#E58B24' },
        green: { 500: '#69AB3C' },
        white: '#FFFFFF',
        gray: { 50: '#FAFAFA', 100: '#F5F5F5', 200: '#E5E5E5', 400: '#A3A3A3', 500: '#737373', 600: '#525252', 700: '#404040' },
        danger: '#E5245E',
      },
      semanticColors: { primary: '#10BCCA', background: '#F4F4F4', text: '#171717', textSecondary: '#525252', border: '#E5E5E5' },
...
jest.mock('@/assets/png/logo_Makesens_Fondo_oscuro.png', () => 3, { virtual: true });
...
const mockHistoricalConfig = [
  {
    name: 'Temperatura', symbol: '\u{1F321}', unit: '°C',
    measurementIds: ['temperatura'], aggregationFunction: 'mean',
    style: { backgroundColor: { colorHex: '#EFF9FB' }, borderColor: { colorHex: '#10BCCA' } },
    graph: { type: 'line', measurementIds: ['temperatura'], aggregationFunction: 'mean',
      style: { backgroundColor: { colorHex: '#EFF9FB' }, borderColor: { colorHex: '#10BCCA' } } },
  },
];
...
const mockMeasurementConfig = {
  tasks: {}, flows: {}, guides: {},
  measurements: { temperatura: { name: 'Temperatura', unit: '°C', range: { min: 10, max: 45 } } },
  bonus: {},
...
// ─── DataStore mocks ──────────────────────────────────────────────────────────
const mockGetLastUserProgressPure = jest.fn().mockResolvedValue({ Seed: 10, Streak: 3, ts: new Date().toISOString() });
const mockGetCompletedTasksByMonthYear = jest.fn().mockResolvedValue({ daysComplete: [], daysIncomplete: [], daysSaveStreak: [] });
...
jest.mock('@/domain/report/environmental-report', () => {
  ...
  function calculatePeriodStats(measurements: Array<{ ts: string; data: string | Record<string, number> | null | undefined }>) {
    if (measurements.length === 0) return { tempMax: null, tempMin: null, humMax: null, humMin: null };
    const temps: number[] = [];
    const hums: number[] = [];
    for (const m of measurements) {
      try {
        const data: Record<string, number> | null = typeof m.data === 'string' ? JSON.parse(m.data) : (m.data as Record<string, number> | null);
        if (!data) continue;
        for (const key of ['TEMPERATURA_MAX', 'TEMPERATURA_MIN', 'temperature', 'temp', 'Temperature']) { if (data[key] !== undefined) temps.push(data[key]); }
        for (const key of ['HUMEDAD_MAX', 'HUMEDAD_MIN', 'humidity', 'hum', 'Humidity']) { if (data[key] !== undefined) hums.push(data[key]); }
      } catch { /* ignore */ }
    }
    return {
  ...
  function calculateTotalRainfall(measurements: Array<{ ts: string; data: string | Record<string, number> | null | undefined }>): number {
    let total = 0;
    for (const m of measurements) {
      try {
        const data: Record<string, number> | null = typeof m.data === 'string' ? JSON.parse(m.data) : (m.data as Record<string, number> | null);
        if (!data) continue;
        for (const key of ['PRECIPITACION', 'rain', 'rainfall', 'Rain']) { if (data[key] !== undefined) total += data[key]; }
      } catch { /* ignore */ }
    }
    return Math.round(total * 10) / 10;
  }

  function groupMeasurementsByDay(measurements: Array<{ ts: string; data: unknown }>): Record<string, Array<{ ts: string; data: unknown }>> {
    const grouped: Record<string, Array<{ ts: string; data: unknown }>> = {};
    for (const m of measurements) {
  ...
  function processDayMeasurements(measurements: Array<{ ts: string; data: string | Record<string, number> | null | undefined }>) {
  ...
  function processDailyData(measurements: Array<{ ts: string; data: string | Record<string, number> | null | undefined }>, year: number, month: number) {
    const daysInMonth = new Date(year, month + 1, 0).getDate();
    const byDay = groupMeasurementsByDay(measurements);
    const days = [];
    for (let day = 1; day <= daysInMonth; day++) {
      const dayKey = `${year}-${String(month + 1).padStart(2, '0')}-${String(day).padStart(2, '0')}`;
      const dayMeasurements = (byDay[dayKey] ?? []) as Array<{ ts: string; data: string | Record<string, number> | null | undefined }>;
      days.push(processDayMeasurements(dayMeasurements));
    }
    return days;
  }

  function calculateSummary(days: Array<{ day: { tempMax: number | null; tempMin: number | null; humMax: number | null; humMin: number | null }; night: { tempMax: number | null; tempMin: number | null; humMax: number | null; humMin: number | null }; rainfall: number | null }>) {
    const allTemps: number[] = [];
    const allHums: number[] = [];
    let totalRainfall = 0;
    let rainyDays = 0;
    for (const day of days) {
      for (const v of [day.day.tempMax, day.day.tempMin, day.night.tempMax, day.night.tempMin]) { if (v !== null && v !== undefined) allTemps.push(v); }
      for (const v of [day.day.humMax, day.day.humMin, day.night.humMax, day.night.humMin]) { if (v !== null && v !== undefined && v >= 0) allHums.push(v); }
      if (day.rainfall !== null && day.rainfall !== undefined) { totalRainfall += day.rainfall; if (day.rainfall > 0) rainyDays++; }
    }
    ...
    return {
      ...
      rainyDays,
      temperature: { max: allTemps.length > 0 ? Math.max(...allTemps) : null, min: allTemps.length > 0 ? Math.min(...allTemps) : null, avg: avg(allTemps) },
      humidity: { max: allHums.length > 0 ? Math.max(...allHums) : null, min: allHums.length > 0 ? Math.min(...allHums) : null, avg: avg(allHums) },
    };
  }

  return {
    EnvironmentalReportService: {
      generateReportData: (...args: unknown[]) => mockGenerateReportData(...args),
      processDailyData,
...
function makeFilledReportData(): ReportData {
  const days = Array.from({ length: 31 }, (_, i) => ({
    day: { tempMax: 28 + i % 5, tempMin: 20 + i % 4, humMax: 85 - i % 10, humMin: 56 + i % 8 },
    night: { tempMax: 26 + i % 3, tempMin: 18 + i % 2, humMax: 90 - i % 8, humMin: 60 + i % 5 },
...
describe('EnvironmentalReportService — processDailyData', () => {
  ...
  it('processes daily data correctly for May 1 with morning measurement', () => {
    ...
    const m = [
      { ts: '2026-05-01T05:00:00.000Z', data: JSON.stringify({ TEMPERATURA_MAX: 30 }) },
    ];
...
describe('EnvironmentalReportService — calculatePeriodStats', () => {
  it('returns nulls for empty array', () => {
    ...
    expect(stats).toEqual({ tempMax: null, tempMin: null, humMax: null, humMin: null });
  });

  it('reads TEMPERATURA_MAX / TEMPERATURA_MIN field variants', () => {
    const m = [{ ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ TEMPERATURA_MAX: 32, TEMPERATURA_MIN: 18 }) }];
  ...
  it('reads temperature / humidity lowercase field variants', () => {
    const m = [{ ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ temperature: 25, humidity: 70 }) }];
    const stats = ActualService.calculatePeriodStats(m);
  ...
  it('reads HUMEDAD_MAX / HUMEDAD_MIN field variants', () => {
    const m = [{ ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ HUMEDAD_MAX: 90, HUMEDAD_MIN: 55 }) }];
    const stats = ActualService.calculatePeriodStats(m);
  ...
  it('calculates max/min across multiple entries', () => {
    const m = [
      { ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ TEMPERATURA_MAX: 28 }) },
      { ts: '2026-05-01T09:00:00Z', data: JSON.stringify({ TEMPERATURA_MAX: 32 }) },
    ];
    const stats = ActualService.calculatePeriodStats(m);
    expect(stats.tempMax).toBe(32);
...
describe('EnvironmentalReportService — calculateTotalRainfall', () => {
  ...
  it('reads PRECIPITACION variant', () => {
    const m = [{ ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ PRECIPITACION: 5.5 }) }];
    expect(ActualService.calculateTotalRainfall(m)).toBe(5.5);
  });

  it('reads rain variant', () => {
    const m = [{ ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ rain: 3.2 }) }];
    expect(ActualService.calculateTotalRainfall(m)).toBe(3.2);
  });

  it('sums across multiple measurements', () => {
    const m = [
      { ts: '2026-05-01T08:00:00Z', data: JSON.stringify({ PRECIPITACION: 5 }) },
...
describe('EnvironmentalReport component — smoke tests', () => {
  ...
  it('renders without crashing', async () => {
    await expect(render(<EnvironmentalReport reportData={reportData} />)).resolves.toBeTruthy();
  });

  it('renders testID="environmental-report"', async () => {
    const { getByTestId } = await render(<EnvironmentalReport reportData={reportData} />);
    expect(getByTestId('environmental-report')).toBeTruthy();
  });

  it('shows the month name', async () => {
    const { getByText } = await render(<EnvironmentalReport reportData={reportData} />);
  ...
  it('renders with all-null summary without crashing', async () => {
    ...
    await expect(render(<EnvironmentalReport reportData={emptyReport} />)).resolves.toBeTruthy();
  });
});
...
describe('HistoricalScreen — B15-cierre image share flow', () => {
  ...
  beforeEach(() => {
    ...
    mockGetCompletedTasksByMonthYear.mockResolvedValue({ daysComplete: [], daysIncomplete: [], daysSaveStreak: [] });
