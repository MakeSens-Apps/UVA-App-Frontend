...
function installBackHandlerHarness(): void {
  ...
  jest
    ...
    .mockImplementation((eventName: 'hardwareBackPress', handler: BackListener) => {
...
jest.mock('@/screens/splash/SplashScreen', () => {
  ...
  return {
    SplashScreen: ({ onAuthResolved }: { onAuthResolved?: (d: string) => void }) => {
      R.useEffect(() => {
        onAuthResolved?.((globalThis as { __TEST_DESTINATION__?: string }).__TEST_DESTINATION__ ?? 'app');
      }, [onAuthResolved]);
...
jest.mock('@/navigation/useAuthGate', () => ({
  useAuthGate: () => ({ destination: null, isChecking: false, startAuthCheck: jest.fn() }),
}));
...
jest.mock('@/navigation/AppStack', () => {
  ...
  const HomeWithProbe = () => {
    ...
    R.useEffect(() => {
      ...
      (globalThis as { __ROOT_STATE__?: unknown }).__ROOT_STATE__ = current.getState();
    });
  ...
  const AppTabsNavigator = () =>
    R.createElement(
      ...
      R.createElement(Tabs.Screen, { name: 'HomeStack', component: HomeStackNavigator }),
  ...
  return {
    AppStack: () =>
      R.createElement(
        Outer.Navigator,
        { screenOptions: { headerShown: false } },
        R.createElement(Outer.Screen, { name: 'AppTabs', component: AppTabsNavigator }),
    ...
    AuthStack: () =>
      R.createElement(
        ...
        R.createElement(S.Screen, {
          name: 'Login',
          children: () => R.createElement(View, null, R.createElement(Text, null, 'Login')),
        }),
      ),
  };
});
...
import { requestNavigateToApp, registerGateSetter } from '@/navigation/navigationGate';
import {
...
describe('B1 — back del sistema en una ruta raíz minimiza (no navega)', () => {
  beforeEach(() => {
    ...
    (globalThis as { __TEST_DESTINATION__?: string }).__TEST_DESTINATION__ = 'app';
  });
  ...
  it('tras el login el estado del root contiene SOLO el navigator App', async () => {
    ...
    (globalThis as { __TEST_DESTINATION__?: string }).__TEST_DESTINATION__ = 'login';
    ...
    const rootState = (globalThis as { __ROOT_STATE__?: { routes: { name: string }[]; routeNames?: string[] } })
      .__ROOT_STATE__;
