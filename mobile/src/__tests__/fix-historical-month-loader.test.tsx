...
const MONTH_A_MEASUREMENTS = [
  { data: { temperatura: 20 }, ts: new Date(CURRENT_YEAR, CURRENT_MONTH, 5, 10).toISOString() },
  { data: { temperatura: 30 }, ts: new Date(CURRENT_YEAR, CURRENT_MONTH, 6, 10).toISOString() },
];

// Month B (previous month, target of the "←" arrow): daily avgs 10 and 14 → avg 12, max 14, min 10.
const MONTH_B_MEASUREMENTS = [
  { data: { temperatura: 10 }, ts: new Date(PREV_MONTH_YEAR, PREV_MONTH, 5, 10).toISOString() },
  { data: { temperatura: 14 }, ts: new Date(PREV_MONTH_YEAR, PREV_MONTH, 6, 10).toISOString() },
];
...
describe('Historial — loader de contenido al cambiar de mes (device review 2026-09-10)', () => {
  it('flecha ← con carga lenta: muestra el loader (no el valor anterior) y luego el nuevo valor', async () => {
    ...
    await waitFor(() => {
      expect(getByTestId('variable-avg-Temperatura').props.children).toBe('25°C');
    });
    ...
    expect(getByTestId('variable-avg-Temperatura').props.children).not.toBe('25°C');
    ...
    await waitFor(() => {
      expect(getByTestId('variable-avg-Temperatura').props.children).toBe('12°C');
    });
