...
jest.mock('@/domain/setup/setup', () => ({
  SetupService: {
    ...
    currentAuthenticatedUser: (...a: unknown[]) => mockCurrentAuthenticatedUser(...a),
...
function resetAllMocks() {
  ...
  mockDownloadAndStoreMoonPhaseData.mockResolvedValue({ success: true, data: true });
}
...
describe('OtpScreen — validateForm (login branch)', () => {
  ...
  it('shows error when register confirmSignUp returns false', async () => {
    ...
    expect(mockNavigate).not.toHaveBeenCalledWith('ValidateCode', expect.anything());
  });
