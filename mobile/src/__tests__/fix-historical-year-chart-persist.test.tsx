...
jest.mock('@/theme/ThemeProvider', () => ({
  useTheme: () => ({
    theme: {
      colors: {
        blue: { 50: '#EFF9FB', 200: '#a5f3fc', 500: '#10BCCA', 600: '#0da8b6', 700: '#14788A', 900: '#164551' },
        green: { 500: '#6dbb63', 700: '#4a9c40' },
        white: '#FFFFFF',
        gray: { 50: '#FAFAFA', 100: '#f3f4f6', 200: '#e5e7eb', 300: '#d1d5db', 400: '#9ca3af', 500: '#6b7280' },
    ...
    return (
      <View testID="calendar-mock">
        <TouchableOpacity
          ...
          onPress={() => onDayPress?.({ date: new Date('2026-05-10'), state: 'complete' })}
        >
          <Text>Day</Text>
        </TouchableOpacity>
      </View>
...
jest.mock('@/components/areachart/Areachart', () => ({
  Areachart: ({ borderColor, chartData }: { borderColor?: string; chartData?: number[] }) => {
...
const RAW_MEASUREMENTS = [
  { data: { temperatura: 25, humedad: 60 }, ts: new Date(CURRENT_YEAR, CURRENT_MONTH, 5, 10).toISOString() },
  { data: { temperatura: 27, humedad: 65 }, ts: new Date(CURRENT_YEAR, CURRENT_MONTH, 6, 10).toISOString() },
];
...
describe('Historial — Año → mes conserva modo gráfica y variable (deviación 2026-09-10)', () => {
  it('gráfica con Humedad seleccionada → Año → tocar un mes → sigue en gráfica con Humedad', async () => {
    const { getByTestId, queryByTestId, getAllByTestId } = await render(<HistoricalScreen />);
    ...
    await waitFor(() => {
      expect(getByTestId('areachart-border-color').props.children).toBe('#10BCCA');
    });

    // Select Humedad explicitly.
    fireEvent.press(getByTestId('variable-card-Humedad'));
    await waitFor(() => {
      expect(getByTestId('areachart-border-color').props.children).toBe('#6dbb63');
    });
    ...
    expect(mockGetMeasurementsByMont).toHaveBeenCalledWith(CURRENT_YEAR, CURRENT_MONTH);
  });
