...
export function MoonPhaseScreen(): React.JSX.Element {
  ...
  useEffect(() => {
    ...
    const loadMoonData = async () => {
      try {
        ...
        const [responsePhase, responseMonth, responseEvents] =
          await Promise.all([
        ...
        if (responseEvents.success) {
          ...
          const formattedEvents = responseEvents.data.map(
            (event: MoonEvent) => ({
              type: event.type,
  ...
  return (
    <View
      style={[styles.container, { backgroundColor: theme.colors.blue[900] }]}
    >
      ...
      >
        ...
        */}
        {moonEvents.length > 0 && (
          <View
            style={[styles.cards, { backgroundColor: theme.colors.blue[800] }]}
          >
