...
export function AlertsScreen({ navigation }: Props): React.JSX.Element {
  ...
  const renderEmpty = () => (
    <View style={styles.emptyContainer} testID="no-notifications">
      ...
      <Text
        style={[styles.emptyTitle, { fontFamily: fontFamilyForWeight('600') }]}
      >
  ...
  const renderItem = ({ item }: { item: GamificationNotification }) => {
    ...
    const iconColor = (() => {
      switch (item.type) {
        case 'seeds':
          return '#c9680e'; // icon-bg-accent / uva_orange-500-shade
        case 'streak':
        ...
        case 'achievement':
          return '#14788A'; // icon-bg-primary / uva_green-700
        case 'bonus':
          return '#164551'; // icon-bg-yellow / Colors-Blue-900
        default:
          return '#737373'; // icon-bg-muted / Colors-Gray-700
      }
    })();
