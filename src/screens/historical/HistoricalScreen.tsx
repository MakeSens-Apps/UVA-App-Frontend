...
export function HistoricalScreen(): React.JSX.Element {
  ...
  const [completedTaskYear, setCompletedTaskYear] = useState<
    CompleteTaskHistorical[]
  >([]);
  const [variables, setVariables] = useState<Historical[]>([]);
  const [measuresConfig, setMeasuresConfig] = useState<MeasurementModel | null>(
    null,
  );
  ...
  const initializeCompletedTasks = useCallback(
    async (mounted: boolean) => {
  ...
  const initializeVariables = useCallback(
    async (
      ...
      if (tf === 'year') {
        ...
        measurementValues =
          await MeasurementDSService.getMeasurementsByDateRange(
            startDate,
            endDate,
          );
      } else {
        ...
        setVariables(
          (prevVariables) =>
            newVariables.map((newVar) => {
              const existingVar = prevVariables.find(
  ...
  useEffect(() => {
    ...
    const loadMonth = async () => {
      try {
        ...
        if (!mounted || typeView !== 'chart' || freshVariables.length === 0)
          return;
  ...
  const changeColorChart = useCallback(
    (measurement: Historical) => {
      setTypeView('chart');
      setVariables((v) =>
  ...
  const goToMonth = useCallback(
    (index: number) => {
      ...
      const month = completedTaskYear.find(
        (h) => h.mes === newIndex && h.date.getFullYear() === newYear,
      );
  ...
  const shareMonthlyReport = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn(
          '[HistoricalScreen] generateReportData failed, falling back to text:',
          dataErr,
        );
      ...
      try {
        ...
        console.warn(
          '[HistoricalScreen] captureRef failed, falling back to text:',
          captureErr,
        );
      }
  ...
  return (
    <View
      style={[styles.container, { backgroundColor: theme.colors.gray[100] }]}
    >
      <Header
        ...
        {!loading && timeFrame === 'month' && (
          ...
          >
            ...
            <View
              style={[
                styles.variablesRow,
                monthLoading && styles.variablesRowLoading,
              ]}
            >
                ...
                >
                  ...
                  >
                    <Text
                      style={[
                        styles.variableAvg,
                        {
                          fontFamily: fontFamilyForWeight(
                            typeView === 'chart' && variable.selected
                              ? '700'
                              : '600',
                          ),
                    ...
                    >
                      ...
                      {monthLoading
                        ? LOADING_STAT_PLACEHOLDER
                        : `${formatStat(variable.avg)}${variable.unit}`}
                    </Text>
                  </View>
                  <View style={styles.variableStat}>
                    ...
                    >
                      {monthLoading
                        ? LOADING_STAT_PLACEHOLDER
                        : `${formatStat(variable.max)}${variable.unit}`}
                    </Text>
                  </View>
                  <View style={styles.variableStat}>
                    <Text
                    ...
                    >
                      {monthLoading
                        ? LOADING_STAT_PLACEHOLDER
                        : `${formatStat(variable.min)}${variable.unit}`}
                    </Text>
                  </View>
                </TouchableOpacity>
              ))}
            ...
            <View style={styles.calendarContainer}>
              {monthLoading ? (
                <View
                  ...
                  testID="historical-month-loading"
                >
                  <ActivityIndicator
                    color={theme.colors.blue[500]}
                    size="large"
            ...
            {completedTaskMonth && (
              <View style={styles.monthNav}>
                ...
                <TouchableOpacity
                  style={[
                    styles.monthNavBtn,
                    completedTaskMonth.mes === 11 &&
                      isNextYearDisabled() &&
                      styles.monthNavBtnDisabled,
                  ]}
                  onPress={() => goToMonth(completedTaskMonth.mes + 1)}
                  disabled={
