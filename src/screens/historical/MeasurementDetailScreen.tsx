...
import {
  ...
  SortDirection,
} from '@/data/datastore/user-progress-ds';
...
export function MeasurementDetailScreen({ route }: Props): React.JSX.Element {
      ...
      try {
        ...
        if (isAllComplete) {
          try {
            ...
            const hasSaveStreak = progressForDay.some(
              (p) => p.SaveStreak === true,
            );
            if (hasSaveStreak) {
              newState = 'saveStreak';
  ...
  return (
    <View
      style={[styles.container, { backgroundColor: theme.colors.blue[50] }]}
    >
      <Header
