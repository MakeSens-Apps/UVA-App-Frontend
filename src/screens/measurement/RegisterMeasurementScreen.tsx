 ...
 */
export function RegisterMeasurementScreen({
  route,
  navigation,
    ...
    async (config: MeasurementModel, fId: string, showBack: boolean) => {
      ...
      const builtMeasurements: LocalMeasurement[] = await Promise.all(
        flowData.measurements.map(async (measurementKey) => {
          const measurementComplete = {
            ...config.measurements[measurementKey],
          };
          const localM: LocalMeasurement = {
            ...
            fieldsArray: Array.from(
              { length: measurementComplete.fields ?? 0 },
              () => '',
            ),
          };
      ...
      if (flowData.guides.length > 0) {
        ...
        if (config.guides[firstGuideKey]?.showAutomatic !== false) {
          setTimeout(() => {
            navigation.navigate('GuideMeasurement', {
              taskId,
              guideKey: firstGuideKey,
            });
          }, 0);
        }
      } else {
   ...
   */
  const onDigitsFocus = useCallback((measIdx: number, digitIdx: number) => {
  ...
  const runValidateRestriction = useCallback(async (): Promise<boolean> => {
    ...
    const lastMeasurementValuesRaw = await Preferences.get({
    ...
    if (!result.valid) {
      ...
      const screenIndex = failureRestriction
        ? measurements.findIndex(
            (m) =>
              m.id !== undefined &&
              failureRestriction.measurementIds.includes(m.id as string),
          )
        : -1;
      const failedIdx =
        screenIndex !== -1 ? screenIndex : (result.failedMeasurementIndex ?? 0);
    ...
    if (!flow?.nextFlow) {
      ...
      setTimeout(async () => {
        ...
        try {
          ...
          console.error(
            'RegisterMeasurementScreen ~ completeTaskProcess error:',
            err,
          );
        }
  ...
  const confirmSave = useCallback(async () => {
    ...
    try {
      ...
      try {
        ...
        if (
          Array.isArray(todaysMeasurements) &&
          flowMeasurementIds.length > 0
        ) {
          alreadySavedToday = todaysMeasurements.some((lazyMeasurement) => {
            if (lazyMeasurement?.task !== taskId || !lazyMeasurement?.data)
              return false;
            const parsed =
        ...
        console.error(
          'RegisterMeasurementScreen ~ duplicate check failed:',
          err,
        );
      }
  ...
  return (
    ...
    >
      <Header
        ...
        onBackPress={() =>
          navigation.navigate('AppTabs', { screen: 'Measurement' })
        }
      />
                      ...
                      <Text
                        style={[
                          styles.alertTitle,
                          {
                            ...
                            color: theme.colors.orange[500] ?? '#E58B24',
                          },
                        ]}
                      >
      ...
      >
        ...
        >
          ...
          <View
            style={[
              styles.modalScrim,
              { opacity: CONFIRM_MODAL_BACKDROP_OPACITY },
            ]}
            pointerEvents="none"
