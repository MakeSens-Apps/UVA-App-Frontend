...
export function MeasurementScreen(): React.JSX.Element {
  ...
  const [bonusConfig, setBonusConfig] = useState<Record<string, Bonus> | null>(
    null,
  );
  ...
  const loadMeasurementData = useCallback(
    async (mounted: boolean) => {
      ...
      if (dataBonusConfig) {
        ...
        if (
          weekOfMonthName &&
          dataBonusConfig['moniliacisis']?.schedule.occurrences.includes(
            weekOfMonthName,
          )
        ) {
          if (dataBonusConfig['moniliacisis'].months.includes(month)) {
            ...
            if (
              dayOfWeekName &&
              dataBonusConfig['moniliacisis'].schedule.daysOfWeek.includes(
                dayOfWeekName,
              )
            ) {
      ...
      const grouped = groupRemainingLazyMeasurements(
        localCompleted,
        completedRaw,
      );
      ...
      groupedKeys.forEach((taskId) => {
        ...
        (task.flows ?? []).forEach((flowKey) => {
          ...
          if (
            flowMeasIds.length > 0 &&
            flowMeasIds.every((id) => savedIds.has(id))
          ) {
        ...
        if (allFlowsComplete) {
          ...
          grouped[taskId].forEach((measurement) => {
            const measurementData =
              configMeasurement.measurements[measurement.id];
            if (measurementData) {
          ...
          remainingTasks[indexTask] = {
            ...task,
            flowsComplete: completedFlows,
          };
        }
      });
