...
import type {
  SystemStatus,
  NotificationState,
} from '@/native/notifications/LocalRemindersService';
...
export function ConfigurationScreen({ navigation }: Props): React.JSX.Element {
  ...
  const [isConfigurationAvailable, setIsConfigurationAvailable] =
    useState(false);
  ...
  const loadNotificationStatus = useCallback(async (): Promise<void> => {
    try {
      ...
      showToast({
        message: 'Error al verificar estado de notificaciones',
        type: 'error',
      });
    }
  }, []);
  ...
  const getStatusChipColor = (): string => {
    if (!notificationSystemStatus.permissionsGranted)
      return theme.colors.danger;
    if (notificationSystemStatus.batteryOptimized)
  ...
  const getNotificationStatusText = (): string => {
    ...
    if (notificationSystemStatus.canSchedule)
  ...
  return (
    <View
      style={[styles.root, { backgroundColor: theme.colors.white }]}
      testID="configuration-screen"
    >
                ...
                >
                  ...
                  >
                    ...
                    <ChevronDownIcon
                      ...
                      style={
                        showNotificationDetails ? styles.chevronUp : undefined
                      }
                    />
                  </TouchableOpacity>
                  ...
                  {showNotificationDetails && (
                    ...
                    >
                      ...
                      >
                        {'• Programación: '}
                        {notificationState.notificationsScheduled
                          ? 'Activa'
                          : 'No programadas'}
                      </Text>
