...
export class EnvironmentalReportService {
  ...
  static async generateReportData(
    ...
    try {
      ...
      if (uva?.fields) {
        const fields =
          typeof uva.fields === 'string' ? JSON.parse(uva.fields) : uva.fields;
    ...
    const days = EnvironmentalReportService.processDailyData(
      rawMeasurements as RawMeasurement[],
      year,
      month,
    );
  ...
  static processDailyData(
    ...
    const measurementsByDay =
      EnvironmentalReportService.groupMeasurementsByDay(measurements);
    const days: DayData[] = [];
    ...
    for (let day = 1; day <= daysInMonth; day++) {
      ...
      days.push(
        EnvironmentalReportService.processDayMeasurements(dayMeasurements),
      );
    }
   ...
   */
  static groupMeasurementsByDay(
    measurements: RawMeasurement[],
  ): Record<string, RawMeasurement[]> {
