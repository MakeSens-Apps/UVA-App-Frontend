...
const PreferencesMock = (
  jest.requireMock('@/data/storage/preferences') as {
    Preferences: {
      get: jest.Mock;
      set: jest.Mock;
      remove: jest.Mock;
  ...
  describe('when current time is BEFORE both reminder hours', () => {
    it('returns today for morning (6h) and afternoon (18h)', () => {
      ...
      const { morning, afternoon } =
        localRemindersService.getNextReminderTimes(now);
...
describe('cancelAllNotifications — R-48 fix', () => {
  it('calls cancelAllScheduledNotificationsAsync (not just removeAllListeners)', async () => {
    ...
    expect(
      NotificationsMock.cancelAllScheduledNotificationsAsync,
...
describe('scheduleDailyNotifications', () => {
  it('schedules two notifications (morning + afternoon) when not programmed', async () => {
    ...
    expect(NotificationsMock.scheduleNotificationAsync).toHaveBeenCalledTimes(
      2,
    );
  });
...
describe('createNotificationChannels', () => {
  ...
  it('omits `sound` so the channel keeps the OS default notification sound', async () => {
    ...
    expect(NotificationsMock.setNotificationChannelAsync).toHaveBeenCalledTimes(
      1,
    );

    const [, options] =
      NotificationsMock.setNotificationChannelAsync.mock.calls[0];
