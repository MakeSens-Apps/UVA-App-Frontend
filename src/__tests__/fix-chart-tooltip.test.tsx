...
describe('formatTooltipDate — dd/MM/yyyy', () => {
  it('rellena día y mes a dos dígitos', () => {
    expect(formatTooltipDate(new Date(2026, 4, 1).getTime())).toBe(
      '01/05/2026',
    );
    expect(formatTooltipDate(new Date(2026, 11, 9).getTime())).toBe(
      '09/12/2026',
    );
    expect(formatTooltipDate(new Date(2026, 9, 31).getTime())).toBe(
      '31/10/2026',
    );
  });
...
describe('buildTooltipContent', () => {
  ...
  it('no inventa desviación estándar (el original no la tiene)', () => {
    ...
    const all = [...detailed.lines, ...plain.lines]
      .map((l) => l.text)
      .join(' ');
