  ...
  it('con los dos flujos guardados, la tarea pasa a "Registros completados"', async () => {
    mockGetMeasurementsByDay.mockResolvedValue([
      SAVED_MAXIMOS,
      {
        task: 'task1',
        ts: new Date().toISOString(),
        data: JSON.stringify({ temperatura_min: 14 }),
      },
    ]);
    ...
    await waitFor(() =>
      expect(getByTestId('task-completed-task1')).toBeTruthy(),
    );
...
describe('#3 — backdrop del modal "Verifica los datos" / "… guardados"', () => {
  ...
  async function openConfirm() {
    ...
    await waitFor(() =>
      expect(utils.getByTestId('digit-input-0-0')).toBeTruthy(),
    );
    await act(async () => {
    ...
    await waitFor(() =>
      expect(utils.getByTestId('confirm-save-button')).toBeTruthy(),
    );
    return utils;
  }
  ...
  it('la capa oscura no captura toques ni tapa el contenido del modal', async () => {
    const { getByTestId } = await openConfirm();
    expect(getByTestId('modal-backdrop-scrim').props.pointerEvents).toBe(
      'none',
    );
