...
jest.mock(
  '@/assets/svg/icons/checkmark-circle.svg',
  () => 'CheckmarkCircleIcon',
);
...
jest.mock('@/native/notifications/LocalRemindersService', () => ({
  localRemindersService: {
    getEnableNotifications: () =>
      Promise.resolve(mockGetEnableNotificationsResult),
    getSystemStatus: () => Promise.resolve(mockGetSystemStatusResult),
    getComprehensiveNotificationState: () =>
      Promise.resolve(mockGetComprehensiveStateResult),
    setEnableNotifications: (...a: unknown[]) =>
      mockSetEnableNotifications(...a),
...
describe('screen structure', () => {
  ...
  it('header title "Configuración" is NOT force-centered (original right-aligns it)', async () => {
    ...
    const flatStyle = titleEl.props.style as Record<string, unknown>[];
  ...
  it('renders notification description text', async () => {
    ...
    expect(getByText(/Al mantener las notificaciones activas/)).toBeTruthy();
  });
});
