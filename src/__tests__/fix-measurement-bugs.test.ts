...
describe('[fix] RegisterMeasurementScreen init — uses flowId param when provided', () => {
  ...
  function simulateInit(
    ...
    const isFirst =
      initialFlowId === undefined && resolvedFlowId === task.flows[0];
  ...
  it('would have been buggy without fix: always flows[0]', () => {
    ...
    function buggyInit(task: Task): string {
      return task.flows[0];
    }
...
describe('[fix] MeasurementScreen goToRegister — picks first incomplete flow', () => {
  ...
  function simulateGoToRegister(task: Task): { flowId: string } | null {
    ...
    const flowId =
      task.flows.find((f) => !task.flowsComplete?.includes(f)) ?? task.flows[0];
    return { flowId };
  }
  ...
  it('would have been buggy without fix: always flows[0]', () => {
    // OLD buggy code:
    function buggyGoToRegister(task: Task) {
      return { flowId: task.flows[0] };
    }
    const taskWithFlow1Complete: Task = {
      flows: ['flow1', 'flow2'],
      flowsComplete: ['flow1'],
    };
...
describe('[fix] GuideMeasurementScreen — nextGuide chaining via replace()', () => {
  ...
  type NavAction =
    | { type: 'replace'; params: { guideKey: string } }
    | { type: 'goBack' };
  ...
  it('navigates to nextGuide via replace when button OK and nextGuide exists', () => {
    ...
    expect(
      (action as { type: 'replace'; params: { guideKey: string } }).params
        .guideKey,
    ).toBe('guide2');
  });
...
describe('[fix] validateRestriction — failedMeasurementIndex maps to screen measurement', () => {
  ...
  function remapFailedIndex(
    ...
    const screenIndex = failureRestriction
      ? measurements.findIndex((m) =>
          failureRestriction.measurementIds.includes(m.id),
        )
      : -1;
    return screenIndex !== -1
      ? screenIndex
      : (engineResult.failedMeasurementIndex ?? 0);
  }
  ...
  it('finds humedad at index 1 when restriction.measurementIds starts with humedad', () => {
    ...
    const screenMeasurements: Measurement[] = [
      { id: 'humedad' },
      { id: 'temperatura' },
    ];
  ...
  it('would have been buggy: engine returned 0 always regardless of actual screen position', () => {
    ...
    function buggyFindIndex(
      restrictionMeasurementIds: string[],
      allValues: { id: string }[],
    ): number {
...
describe('[fix] RegisterMeasurementScreen — range alert shown only when all digits filled', () => {
  ...
  function computeShowAlert(params: {
    ...
    const isOutOfRange =
      allDigitsFilled && (value! < rangeMin || value! > rangeMax);
...
describe('[fix] auto-opened guide uses the current flow guide key (not flows[0])', () => {
  ...
  function autoOpenGuideParams(
    flowKey: string,
    taskId: string,
  ): { taskId: string; guideKey?: string } {
  ...
  it('flow1 opens guide1', () => {
    expect(autoOpenGuideParams('flow1', 'task1')).toEqual({
      taskId: 'task1',
      guideKey: 'guide1',
    });
  });
  ...
  it('flow2 (mínimos) opens guide2 — not guide1', () => {
    expect(autoOpenGuideParams('flow2', 'task1')).toEqual({
      taskId: 'task1',
      guideKey: 'guide2',
    });
  });
  ...
  it('OLD BUG PROOF: passing only { taskId } makes the guide screen fall back to flows[0]=guide1', () => {
    const buggyParams = { taskId: 'task1' } as {
      taskId: string;
      guideKey?: string;
    };
