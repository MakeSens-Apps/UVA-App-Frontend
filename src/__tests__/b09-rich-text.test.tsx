...
import {
  ...
  ALLOWED_ATTR,
  SANITIZE_OPTIONS,
} from '../components/rich-text/sanitize';
// eslint-disable-next-line import/first
import {
  resolveVarToken,
  resolveVarTokensInHtml,
} from '../components/rich-text/varTokenResolver';
// eslint-disable-next-line import/first
import { RichText } from '../components/rich-text/RichText';

function buildConfigMock(
  overrides: Partial<ConfigContextValue> = {},
): ConfigContextValue {
  return {
    configApp: null,
...
describe('resolveVarToken — CSS custom property resolution', () => {
  ...
  it('runtime branding override takes precedence over static fallback', () => {
    ...
    expect(resolveVarToken('ion-color-uva_blue-500', brandingOverrides)).toBe(
      '#FF0000',
    );
  });
...
describe('resolveVarTokensInHtml — HTML var() replacement', () => {
  ...
  it('replaces multiple var() in the same style attribute', () => {
    const input =
      '<div style="color: var(--ion-color-uva_blue-600); background: var(--ion-color-uva_green-500);">x</div>';
    const result = resolveVarTokensInHtml(input);
...
describe('RichText — F-09 device regression (inline styles on native)', () => {
  it('renders the register subtitle with Montserrat-Bold and the resolved green', async () => {
    ...
    expect(
      styles.some(
        (s) => s.fontWeight !== undefined && s.fontWeight !== 'normal',
      ),
    ).toBe(false);
