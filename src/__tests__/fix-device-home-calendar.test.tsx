...
jest.mock('@/data/datastore/user-progress-ds', () => ({
  UserProgressDSService: {
    ...
    recalculateDailyProgress: (...a: unknown[]) =>
      mockRecalculateDailyProgress(...a),
...
jest.mock('@/native/notifications/LocalRemindersService', () => ({
  ...
  localRemindersService: {
    getEnableNotifications: (...a: unknown[]) =>
      mockGetEnableNotifications(...a),
...
describe('Day — device review D-02 / D-06 / D-22 / D-37', () => {
  ...
  it('mini cells keep the original hairline rings so they do not blob (D-39)', async () => {
    ...
    expect(
      flatten(getByTestId('day-cell-3').props.style).borderWidth,
    ).toBeCloseTo(0.661);
  });
...
describe('HomeScreen — device review D-01 / D-04 / D-05 / D-09 / D-10 / D6', () => {
  ...
  it('the weekly calendar sits in a white bordered inner panel (D-01)', async () => {
    ...
    const panel = flatten(
      (await findByTestId('home-calendar-panel')).props.style,
    );
  ...
  it('modal_Days_question shows the FIXED 02..08 example, not the real week (D-10)', async () => {
    ...
    expect(flatten(getByTestId('day-cell-2').props.style).backgroundColor).toBe(
      '#1097AA',
    );
    expect(flatten(getByTestId('day-cell-3').props.style).backgroundColor).toBe(
      'transparent',
    );
    ...
    expect(flatten(getByTestId('day-cell-5').props.style).backgroundColor).toBe(
      '#1097AA',
    );
    expect(flatten(getByTestId('day-cell-7').props.style).backgroundColor).toBe(
      'transparent',
    );
    expect(flatten(getByTestId('day-cell-8').props.style).backgroundColor).toBe(
      'transparent',
    );
...
describe('BottomSheet — device review D-12 / D-13 / D-14', () => {
  ...
  it('keeps the grey drag handle (D-14)', async () => {
    ...
    const props = (globalThis as Record<string, unknown>)
      .__sheetProps as Record<string, unknown>;
