...
describe('F-09 cause 1 — RNRH engine defaults are not inherited from defaultProps', () => {
  ...
  it('mirrors the rest of RNRH defaultTRenderEngineProviderProps', () => {
    ...
    expect(RICH_TEXT_ENGINE_DEFAULTS.htmlParserOptions).toEqual({
      decodeEntities: true,
    });
...
describe('rewriteFontDeclarations', () => {
  it('replaces font-weight with the matching Montserrat family', () => {
    expect(rewriteFontDeclarations('font-weight: 700')).toBe(
      'font-family: Montserrat-Bold',
    );
    expect(rewriteFontDeclarations('font-weight: 600')).toBe(
      'font-family: Montserrat-SemiBold',
    );
    expect(rewriteFontDeclarations('font-weight: 500')).toBe(
      'font-family: Montserrat-Medium',
    );
    expect(rewriteFontDeclarations('font-weight: 400')).toBe(
  ...
  it('combines font-weight and font-style into a single italic face', () => {
    expect(
      rewriteFontDeclarations('font-weight: 700; font-style: italic'),
    ).toBe('font-family: Montserrat-BoldItalic');
  ...
  it('keeps every other declaration untouched', () => {
    expect(
      rewriteFontDeclarations(
        'font-size: 16px; line-height: 150%; font-weight: 700; color: #e58b24',
      ),
    ).toBe(
      'font-size: 16px; line-height: 150%; color: #e58b24; font-family: Montserrat-Bold',
    );
  });
  ...
  it('returns null (no rewrite) when there is nothing to map', () => {
    ...
    expect(
      rewriteFontDeclarations('font-family: Menlo; font-weight: 700'),
    ).toBeNull();
  });
});
