...
const mockLoadImage = jest
  .fn()
  .mockResolvedValue('file:///data/racimo/branding/logo.png');
...
const mockGetMilestones = jest
  .fn()
  .mockResolvedValue(['brote', 'brote', 'plantula']);
...
const mockGetNotifications = jest.fn().mockResolvedValue([
  {
    ...
    data: {
      ...
      description: '✅ Primera tarea del día lista',
      isUnread: true,
    },
    timestamp: 'Hoy 10:00',
    isUnclean: false,
...
jest.mock('@/domain/gamification/gamification', () => ({
  GamificationService: {
    ...
    markNotificationAsRead: (...a: unknown[]) =>
      mockMarkNotificationAsRead(...a),
    deleteAllNotifications: (...a: unknown[]) =>
      mockDeleteAllNotifications(...a),
  },
}));
...
describe('PersonalInfoScreen — validateInput', () => {
  ...
  it('focus visual per-input: getFieldBorderColor retorna azul para el campo enfocado (fix coverage-audit #13)', () => {
    ...
    expect(
      getFieldBorderColor('userPhoneNumber', true, 'userPhoneNumber', true),
    ).toBe('#E5E5E5');
  });
  ...
  it('parseo defensivo de uva.fields: maneja string JSON correctamente', async () => {
    ...
    const validJson =
      '{"farmName":"Finca","villageName":"Vereda","townName":"Municipio"}';
...
describe('AlertsScreen — markAsRead decrements unreadCount', () => {
    ...
    mockGetNotifications.mockResolvedValue([
      {
        ...
        data: {
          title: 'Primera tarea',
          description: 'Primera tarea completada',
          isUnread: true,
        },
        timestamp: 'Hoy',
        isUnclean: false,
  ...
  it('getNotificationIconColor: streak_recovery usa #164551 (Colors-Blue-900), NO #0d8f9a (fix coverage-audit #11)', () => {
    ...
    expect(getColor('streak', 'streak_recovery')).not.toBe(
      getColor('streak', 'streak_recovered'),
    );
