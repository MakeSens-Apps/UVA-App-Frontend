...
const mockReadFile = jest
  .fn()
  .mockResolvedValue({ success: true, data: { data: ARROW_SVG } });
...
const mockLoadImage = jest.fn((path: string) =>
  Promise.resolve(`file:///data/racimo/${path}`),
);
...
function makeRegisterProps(flowId = 'flow1') {
  ...
  const route = {
    params: { taskId: 'task1', taskName: 'Registro máximos', flowId },
  };
  return { navigation, route } as unknown as React.ComponentProps<
...
describe('time restriction — greying out vs navigation (frame 054)', () => {
  ...
  it('reports the 08:00 window as CLOSED at 08:16 local time', () => {
    ...
    expect(getTextRestrictionTime(task('05:00', '08:00'))).toBe(
      'Disponible hasta las 08:00',
    );
  });
  ...
  it('greys the label for a TEST user too — only navigation is bypassed', async () => {
    ...
    expect(flattenStyle<TextStyle>(restrictedLabel?.props.style).color).toBe(
      '#A3A3A3',
    );
    // The unrestricted task keeps --Gray-700.
    expect(flattenStyle<TextStyle>(openLabel?.props.style).color).toBe(
      '#404040',
    );
...
describe('GuideMeasurementScreen — sheet chrome and content', () => {
  it('D-25 — pads the "Entendido" footer by the bottom safe-area inset', async () => {
    ...
    const footer = flattenStyle<ViewStyle>(
      getByTestId('guide-buttons').props.style,
    );
    ...
    const sheet = flattenStyle<ViewStyle>(
  ...
  it('D-26 — puts guide.icon on the right of the title, tinted with colorHex', async () => {
    ...
    const row = flattenStyle<ViewStyle>(
      getByTestId('guide-title-row').props.style,
    );
  ...
  it('D-28 — "Mostrar automaticamente." starts unchecked and persists nothing', async () => {
    ...
    await waitFor(() =>
      expect(getByTestId('guide-show-automatic')).toBeTruthy(),
    );
  ...
  it('D-29 — dims the page above the sheet and shows the drag handle', async () => {
    ...
    const backdrop = flattenStyle<ViewStyle>(
      getByTestId('guide-backdrop').props.style,
    );
    ...
    const handle = flattenStyle<ViewStyle>(
      getByTestId('guide-drag-handle').props.style,
    );
...
describe('RegisterMeasurementScreen — form chrome', () => {
  ...
  it('D-32 — range alert is a white bordered box with a centred title', async () => {
    ...
    await waitFor(() =>
      expect(getByTestId('measurement-alert-0')).toBeTruthy(),
    );

    const box = flattenStyle<ViewStyle>(
      getByTestId('measurement-alert-0').props.style,
    );
...
describe('RegisterMeasurementScreen — confirmation modal (D-35)', () => {
  async function openConfirm() {
    ...
    await waitFor(() =>
      expect(utils.getByTestId('measurement-icon-0')).toBeTruthy(),
    );
    await act(async () => {
    ...
    await waitFor(() =>
      expect(utils.getByTestId('confirm-modal-wrapper')).toBeTruthy(),
    );
    return utils;
  }
  ...
  it('floats over the blurred form with no white container card', async () => {
    ...
    const wrapper = flattenStyle<ViewStyle>(
      getByTestId('confirm-modal-wrapper').props.style,
    );
  ...
  it('shows the value as separated underlined digits, not a joined string', async () => {
    ...
    const box = flattenStyle<ViewStyle>(
      getByTestId('confirm-digit-0-0').props.style,
    );
...
describe('RegisterMeasurementScreen — saved modal (D-36)', () => {
  it('gives "Siguiente" the full width of the modal card plus padding', async () => {
    ...
    const container = flattenStyle<ViewStyle>(
      getByTestId('next-flow-container').props.style,
    );
    ...
    const btn = flattenStyle<ViewStyle>(
      getByTestId('next-flow-button').props.style,
    );
    expect(btn.height).toBe(36);
    expect(btn.borderRadius).toBe(8);
  });
