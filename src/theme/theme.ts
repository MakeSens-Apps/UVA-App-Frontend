...
export function applyOverrideToColors(
  ...
  if (paletteMatch) {
    ...
    if (scale === 'blue' && scale in target) {
      (target.blue as Record<number, string>)[step] = value;
      return;
    }
    if (scale === 'green' && scale in target) {
      (target.green as Record<number, string>)[step] = value;
      return;
    }
    if (scale === 'orange' && scale in target) {
      (target.orange as Record<number, string>)[step] = value;
      return;
    }
    if (scale === 'gray' && scale in target) {
      (target.gray as Record<number, string>)[step] = value;
      return;
    }
    return;
  }
  ...
  if (ionMatch) {
    ...
    if (ionScale === 'blue') {
      (target.blue as Record<number, string>)[ionStep] = value;
      return;
    }
    if (ionScale === 'orange') {
      (target.orange as Record<number, string>)[ionStep] = value;
      return;
    }
    if (ionScale === 'green') {
      // Special case: uva_green-700 is actually blue (documented alias in theme.ts / R-42)
      if (ionStep === 700) {
        (target.blue as Record<number, string>)[700] = value;
        return;
      }
