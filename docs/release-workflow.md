...
3. Un config plugin de Expo (ver `plugins/`) inyecta la configuración de firma (`signingConfigs.release` en el `android/app/build.gradle` generado) durante `expo prebuild`, leyendo la ruta del keystore y las credenciales desde variables de entorno.
...
1. Actualizar `version` y `android.versionCode` en `app.json` (versionCode estrictamente mayor al último publicado).
2. Verificar que `npx tsc --noEmit` y `npx jest --ci` están en verde.
