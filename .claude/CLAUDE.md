...
> The codebase lives at the repo root. The original Ionic/Angular project was removed in the cutover of 2026-09-11 (see `docs/migration/plan.md`); the Ionic code is preserved at the `pre-cutover-2026-09-11` tag and the `V2.x` tags for anyone who needs it. This file, and the code at the repo root, describe the RN result of that migration.
...
- **S3** — file storage
- **AsyncStorage** (`@react-native-async-storage/async-storage`) — local DataStore adapter, SQLite size cap raised to **200 MB** via a config plugin (`plugins/withAsyncStorageDbSize.js`)

### Development Tools

- **ESLint** + **Prettier**
- **Jest** (`jest-expo` preset) + **@testing-library/react-native** for unit/component tests
- **Husky** for git hooks (see `.husky` — owned by a separate migration workstream)
...
- **Config plugins over hand-edited native code**: anything that needs to touch generated `android/` (which `expo prebuild` regenerates and is not committed) goes through a plugin in `plugins/`, never a manual edit under `android/`
...
`amplifyconfiguration.json` is gitignored and required at runtime/build time — without it Metro fails to resolve the import in `src/data/amplify-bootstrap/amplify-config.ts`.
...
- `amplifyconfiguration.json` — Amplify backend config (gitignored, provided out-of-band)
...
This app is the target of the Ionic→React Native migration, cut over to the repo root on 2026-09-11. Full plan, block-by-block status, and verification gates:
